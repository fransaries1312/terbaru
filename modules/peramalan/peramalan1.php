...
switch ($act) {
    ...
    default:
        ...
        function between_month($tgl_akhir)
        {
            ...
            $tgl_akhir=$bulan->modify('+1 month')->format('Y-m-d');
...
//     
    ...
    $tanggal=week_between_two_dates(date('Y-m-d',strtotime($_GET['tanggal_awal'])), date('Y-m-d',strtotime($_GET['tanggal_akhir'])));
        ...
        $result = mysqli_query($koneksi, "SELECT SUM(jumlah) as jumlah,CONCAT(DATE_FORMAT(tanggal, '%Y'),'/',DATE_FORMAT(tanggal, '%m')) AS tahun_bulan FROM data_rekap WHERE id_obat = $_GET[id_obat] and tanggal >= '$tgl_awal' and tanggal <='$tgl_akhir' GROUP BY MONTH(tanggal), YEAR(tanggal) ORDER BY tanggal  ASC");
//         
        ...
        $x=  array_merge($x,between_month($tgl_akhir));
